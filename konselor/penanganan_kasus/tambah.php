                                ...
                                <div class="form-group field item row">
                                    ...
                                    <div class="col-md-6 col-sm-6 ">
                                        <select class="form-control has-feedback-left" name="no_registrasi" id="no_registrasi" required>
                                            ...
                                            <?php  
                                                $sql_konselor = mysqli_query($mysqli, "SELECT * FROM tb_dampak INNER JOIN tb_registrasi ON tb_registrasi.id_registrasi = tb_dampak.id_registrasi WHERE 1 ORDER BY id_dampak DESC") or die (mysqli_error($mysqli));
...
$('.lyb1').click(function()  {
        ...
        if($(this).is(':checked')){
            if (idstatus === 'layanan1') {
                ...
                $('#example1,#keg_kons,#inf_kons').attr('required', true);
            }
        } 
        if(!$(this).is(':checked')) {
            if (idstatus === 'layanan1') {
                $('#hidden_konseling_telp').hide('slow');
                $('#example1,#keg_kons,#inf_kons').removeAttr('required');
            }
        }
});
...
$('.lyb2').click(function()  {
        ...
        if($(this).is(':checked')){
            if (idstatus === 'layanan2') {
                ...
                $('#example2,#keg_konsul,#inf_konsul').attr('required', true);
            }
        } 
        if(!$(this).is(':checked')) {
            if (idstatus === 'layanan2') {
                $('#hidden_konsultasi_hukum').hide('slow');
                $('#example2,#keg_konsul,#inf_konsul').removeAttr('required');
            }
        }
});
...
$('.lyb3').click(function()  {
        ...
        if($(this).is(':checked')){
            if (idstatus === 'layanan3') {
                ...
                $('#example7,#example8,#keg_lit,#inf_lit').attr('required', true);
            }
        } 
        if(!$(this).is(':checked')) {
            if (idstatus === 'layanan3') {
                $('#hidden_litigasi').hide('slow');
                $('#xample7,#example8,#keg_lit,#inf_lit').removeAttr('required');
            }
        }
});
...
$('.lyb4').click(function()  {
        ...
        if($(this).is(':checked')){
            if (idstatus === 'layanan4') {
                ...
                $('#example3,#keg_home,#inf_home').attr('required', true);
            }
        } 
        if(!$(this).is(':checked')) {
            if (idstatus === 'layanan4') {
                $('#hidden_home_visite').hide('slow');
                $('#example3,#keg_home,#inf_home').removeAttr('required');
            }
        }
});
...
$('.lyb5').click(function()  {
        ...
        if($(this).is(':checked')){
            if (idstatus === 'layanan5') {
                ...
                $('#example4,#keg_medis,#inf_medis').attr('required', true);
            }
        } 
        if(!$(this).is(':checked')) {
            if (idstatus === 'layanan5') {
                $('#hidden_medis').hide('slow');
                $('#example4,#keg_medis,#inf_medis').removeAttr('required');
            }
        }
});
...
// LAYANAN YANG DIBERIKAN SUPPORT GROUP
$('.lyb8').click(function()  {
        idstatus = $(this).attr('id');
        if($(this).is(':checked')){
            if (idstatus === 'layanan8') {
                $('#hidden_support_group').show('slow');
                $('#example5,#keg_supp,#inf_supp').attr('required', true);
            }
        } 
        if(!$(this).is(':checked')) {
            if (idstatus === 'layanan8') {
                $('#hidden_support_group').hide('slow');
                $('#example5,#keg_supp,#inf_supp').removeAttr('required');
            }
        }
});

// LAYANAN YANG DIBERIKAN ASPIRASI LAINNYA
$('.lyb10').click(function()  {
        idstatus = $(this).attr('id');
        if($(this).is(':checked')){
            if (idstatus === 'layanan10') {
                $('#hidden_aspirasi_lainnya').show('slow');
                $('#example6,#keg_asp,#inf_asp').attr('required', true);
            }
        } 
        if(!$(this).is(':checked')) {
            if (idstatus === 'layanan10') {
                $('#hidden_aspirasi_lainnya').hide('slow');
                $('#example6,#keg_asp,#inf_asp').removeAttr('required');
            }
        }
});
...
<?php
include ('../../layout/wrapperkonselor/footer.php');
?>
