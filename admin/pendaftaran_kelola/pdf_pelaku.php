...
use Dompdf\Dompdf;
$dompdf = new Dompdf();
$html = '<center><h2>DATA PELAKU P2TPAKK "REKSO DYAH UTAMI"</h2></center>';
$html .='
										...
										if(isset($_POST['pdf'])){
									                ...
									                if(!empty($salah)){
														...
														while ($row = mysqli_fetch_array($sql_id_pelaku)){
															...
															if($row['hubungan_korban'] == "Lainnya"){$hubungan = $row['hubungan_lainnya'];} else {$hubungan = $row['hubungan_korban'];} 
															if($row['status_perkawinan_pelaku'] == "Lainnya"){$status = $row['status_lainnya_pelaku'];} else { $status = $row['status_perkawinan_pelaku'];}
															if($row['pekerjaan_pelaku'] == "Lainnya"){$pekerjaan = $row['pekerjaan_lainnya_pelaku'];} else {$pekerjaan = $row['pekerjaan_pelaku'];} 
														$html .="<tr>";
														$html .="	<td>".$no++.".</td>";
														$html .="	<td>".$row['no_registrasi']."</td>";
														$html .="	<td>".$row['nama_pelaku']."</td>";
														$html .="	<td>".$row['umur']."</td>";
														$html .="	<td>".$row['pendidikan_pelaku']."</td>";
														$html .="	<td>".$pekerjaan."</td>"; 
														$html .="	<td>".$status."</td>";
														$html .="	<td>".$hubungan."</td>";
														$html .="	<td>".$kasus."</td>";
														$html .="	<td>".$lokasi."</td>";
														$html .="	<td>".$row['alamat_pelaku']."</td>";
														...
														$html .="</tr>";
															...
															$html .= "<tr><td colspan=\"14\" align=\"center\"> Data Tidak Ditemukan </td></tr>";
														}
													}
											}
...
$dompdf->render();
$dompdf->stream('Laporan_Pelaku.pdf');
?>
